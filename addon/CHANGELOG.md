...
- Mobile sun arc and time slider match desktop behavior: slider range spans dawn-extension to dusk-extension (the visible arc window), tick labels show Dawn, Solar Noon, and Dusk with actual times updated per date, and the sun ball tracks the slider position; dragging the slider to or past the current time on today snaps back to live mode automatically
- Mobile timeline controls include jump-to-start-of-day, play/pause playback, previous/next day arrows, a tappable date label that opens the native date picker, and a Live button; day navigation preserves the slider's time-of-day position on the new date, matching desktop behavior
- Mobile chart tabs (Grid, Production, Battery) include Today/Week/Month/Year range buttons; tapping a bar or point on any chart shows a color-coded toast with the data for that time slot; tap anywhere to dismiss; Battery tab is hidden when no battery system is configured
- Chart "Today" tab label changes to the short date (e.g. "Jun 25") when viewing a past date via the timeline, and the chart data reflects that date; label resets to "Today" when returning to live mode
- Mobile settings tab exposes refresh interval, min average W, peak panel W, and battery invert toggle with a Save button; layout and panel name options are desktop-only and noted as such

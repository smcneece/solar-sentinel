...
function onLive() {
  ...
  renderPanels(st.panels);
  requestAnimationFrame(fitViewGrid);
  // Reload today's live sun times in case the arc was showing a past date.
  fetchSun().then(() => {
    const [arcMin, arcMax] = sunArcMinutes();
    const slider = document.getElementById('time-slider');
    slider.min = arcMin;
...
async function onDateChange(preserveSlider = false) {
  ...
  // Refresh the sun arc for the selected date (sunrise/sunset/dawn/dusk/noon
  // and moon phase are computed server-side for non-today dates). Pass no date
  // for today so the live sun.sun values are used.
...
async function init() {
  ...
  // Periodic sun refresh: only refresh live data when viewing today, so a
  // past date the user has selected isn't overwritten with today's times.
  setInterval(() => {
    const dp = document.getElementById('date-picker');
